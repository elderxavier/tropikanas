...
actionsBilling.prototype.Run = function(){
    ...
    function bilingAutoComplete(cep){
        ...
        jQuery.ajax({
            ...
            success: function (response) {
                ...
                jQuery('select[name="billing[region_id]"] option:contains("'+uf+'")').prop('selected', true).val(uf);                
                jQuery('select[name="billing[region_id]"] option').each(function() {
                    if(jQuery(this).val() != uf)
                    jQuery(this).remove();
                });
    ...
    window.onscroll = function () {
        if(jQuery('#opc-billing').hasClass('active')){
            var theigth = (document.querySelector("#checkout-step-billing").offsetHeight / 3) * 2;
    ...
    function addValuesbilling() {
        ...
        console.log(document.querySelector('input[name="billing[street][]"]').value);
    }
};
  ...
  jQuery('input[name="shipping[postcode]"]').keypress(function (e) {
	...
	function bilingAutoComplete(cep){
        ...
        jQuery.ajax({
            ...
            success: function (response) {
                ...
                var uf = mapState(response.uf);
                jQuery('select[name="shipping[region_id]"] option:contains("'+uf+'")').prop('selected', true).val(uf);                
    ...
    window.onscroll = function () {
        if(jQuery('#opc-shipping').hasClass('active')){
            var theigth = (document.querySelector("#checkout-step-shipping").offsetHeight / 3) * 2;
            if (document.body.scrollTop >= theigth) {
                addValuesshipping();
            }
        }
    ...
    function addValuesshipping() {
        ...
        console.log(document.querySelector('input[name="shipping[street][]"]').value);
        console.log(document.querySelector('input[name="shipping[street][]"]').value);
    }
};
